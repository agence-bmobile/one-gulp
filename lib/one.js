...
exports.init = function (gulp, options) {
    ...
    one.defaultOptions = {
        ...
        browserSyncProxy: 'localhost:' + getPort(0),
    ...
    one.link(one.transforms.svg.store).to(one.transforms.svg.inject, { primary: false });

    one.link(one.sources.html).to(one.transforms.svg.inject);
    ...
    one.link(one.transforms.svg.inject).to(one.transforms.injectDev);
    ...
    one.link(one.transforms.svg.inject).to(one.transforms.injectProd);
